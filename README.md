# emulator-ts
Node.js implementation of Emulator consosole: https://developer.android.com/studio/run/emulator-console.
...
```
Reads the emulator autentication token.
...
emulator.ping()
    .then(() => null));
```
Pings the emulator port. If resolves, emulator is alive. Analougous to `ping`.
...
```ts
emulator.powerDisplay()
    .then((props: PowerDisplay) => console.log(props));
```
...
##### emulator.geoFix(longitude: number, latitude: number, altitude?: number, satelites?: number, velocity?: number)
```ts
...
##### emulator.avdSnaphotSave(name: string)
```ts
emulator.avdSnaphotSave('some-name')
    .then(() => null));
```
...
Sets emultaor's UI theme to either light or dark.
