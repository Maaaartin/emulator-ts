...
import { EmulatorPorts, EventType, GsmState, NetProtocol, PowerDisplay, PowerStatus, ScreenRecordOptions } from ".";
...
export default class EmulatorClient {
    ...
    static readTokenSync() {
        fs.readFileSync(Path.join(process.env.USERPROFILE, '.emulator_console_auth_token')).toString();
    }

    static readToken() {
        return new Promise<string>((resolve, reject) => {
            fs.readFile(Path.join(process.env.USERPROFILE, '.emulator_console_auth_token'), (err, data) => {
    ...
    ping() {
        return this.connection()
            .then((conn) => {
                return new PingCommand(conn, this.parser)
                    .execute();
            });
    }
        ...
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`automation record ${cwd ? Path.join(cwd, name) : name}`);
            });
    }
    ...
    automationStopRecord() {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`automation stop-record`);
            });
    }
    ...
    automationPlay(name: string, cwd?: string) {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`automation play ${cwd ? Path.join(cwd, name) : name}`);
            });
    }
    ...
    powerDisplay(): Promise<PowerDisplay> {
        return this.connection()
            .then((conn) => {
                return new PowerDisplayCommand(conn, this.parser)
                    .execute();
            });
    }
    ...
    powerPresent(state: boolean) {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`power present ${state}`);
            });
    }

    powerHealth(state: 'unknown' | 'good' | 'overheat' | 'dead' | 'overvoltage' | 'failure') {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`power health ${state}`);
            });
    }
    ...
    powerCapacity(capacity: number) {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`power capacity ${capacity}`);
            });
    }
    ...
    redirAdd(protocol: NetProtocol, hostPort: number, guestPort: number) {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`redir add ${protocol}:${hostPort}:${guestPort}`);
            });
    }
    ...
    redirDel(protocol: NetProtocol, hostPort: number) {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`redir del ${protocol}:${hostPort}`);
            });
    }
    ...
    redirList() {
        return this.connection()
            .then((conn) => {
                return new RedirListCommand(conn, this.parser)
                    .execute();
            });
    }
    ...
    sms(phoneNr: number | string, message: string) {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`sms send ${phoneNr} ${message}`);
            });
    }
    ...
    eventCodes(type: EventType) {
        return this.connection()
            .then((conn) => {
                return new EventCodesCommand(conn, this.parser)
                    .execute(type);
            });
    }

    eventSend(type: 'EV_KEY', code: Ev_KeyCode, value: 0 | 1 | string): Promise<void>;
    eventSend(type: 'EV_REL', code: Ev_RelCode, value: 0 | 1 | string): Promise<void>;
    eventSend(type: 'EV_ABS', code: Ev_AbsCode, value: 0 | 1 | string): Promise<void>;
    eventSend(type: 'EV_SW', code: Ev_SwCode, value: 0 | 1 | string): Promise<void>;
    eventSend(type: EventType, code: string | number, value: 0 | 1 | string): Promise<void>;
    eventSend(type: string | number, code: string | number, value: 0 | 1 | string): Promise<void>;
    eventSend(type: string | number, code: string, value: 0 | 1 | string) {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`event send ${type}:${code}:${value}`);
            });
    }
    ...
    eventMouse(x: number, y: number, device = 0, bntState = 1) {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`event mouse ${x} ${y} ${device} ${bntState}`);
            });
    }
    ...
    sensorStatus() {
        return this.connection()
            .then((conn) => {
                return new SensorStatusCommand(conn, this.parser)
                    .execute();
            });
    }
    ...
    sensorGet(name: string) {
        return this.connection()
            .then((conn) => {
                return new SensorGetCommand(conn, this.parser)
                    .execute(name);
            });
    }
    ...
    physicsRecordGt(name: string, cwd?: string) {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`physics record-gt ${cwd ? Path.join(cwd, name) : name}`);
            });
    }
    ...
    geoNmea(sentence: string, ...params: string[]) {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`geo nmea ${sentence}, ${params.join(',')}`);
            });
    }

    geoFix(longitude: number, latitude: number, altitude?: number, satelites?: number, velocity?: number) {
        return this.connection()
            .then((conn) => {
                return new VoidQueueCommand(conn, this.parser)
                    .addArg(altitude, ' ')
    ...
    gsmList() {
        return this.connection()
            .then((conn) => {
                return new GsmListCommand(conn, this.parser)
                    .execute();
            });
    }
    ...
    gsmStatus() {
        return this.connection()
            .then((conn) => {
                return new GsmStatusCommand(conn, this.parser)
                    .execute();
            });
    }
    ...
    gsmSignalProfile(strength: 0 | 1 | 2 | 3 | 4) {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`gsm signal-profile ${strength}`);
            });
    }
    ...
    cdmaPrlVersion(version: string | number) {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`cdma prl_version ${version}`);
            });
    }
    ...
    networkStatus() {
        return this.connection()
            .then((conn) => {
                return new NetworkStatusCommand(conn, this.parser)
                    .execute();
            });
    }

    networkSpeed(up: number, down: number): Promise<void>;
    networkSpeed(speed: 'gsm' | 'hscsd' | 'gprs' | 'edge' | 'umts' | 'hsdpa' | 'lte' | 'evdo' | 'full'): Promise<void>;
    ...
    networkCaptureStart(name: string, cwd?: string) {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`network capture start ${cwd ? Path.join(cwd, name) : name}`);
            });
    }
    ...
    avdStatus() {
        return this.connection()
            .then((conn) => {
                return new AvdStatusCommand(conn, this.parser)
                    .execute();
            });
    }
    ...
    avdHeartbeat() {
        return this.connection()
            .then((conn) => {
                return new AvdHearbeatCommand(conn, this.parser)
                    .execute();
            });
    }
    ...
    avdName() {
        return this.connection()
            .then((conn) => {
                return new AvdNameCommand(conn, this.parser)
                    .execute();
            });
    }
    ...
    avdSnapshotList() {
        return this.connection()
            .then((conn) => {
                return new AvdSnapshotListCommand(conn, this.parser)
                    .execute();
            });
    }
    ...
    avdSnaphotSave(name: string) {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`avd snapshot save ${name}`);
            });
    }
    ...
    avdSnaphotLoad(name: string) {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`avd snapshot load ${name}`);
            });
    }
    ...
    avdSnaphotDelete(name: string) {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`avd snapshot delete ${name}`);
            });
    }
    ...
    avdSnaphotRemap(autoSave: 0 | 1) {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`avd snapshot remap ${autoSave}`);
            });
    }
    ...
    screenrecordStart(name: string, options?: ScreenRecordOptions & { cwd?: string }) {
        options = options || {};
        ...
        return this.connection()
            .then((conn) => {
                return new VoidQueueCommand(conn, this.parser)
                    .setTimeout(100)
                    .addArg(height !== undefined && width !== undefined ? `--size ${width}x${height}` : undefined, ' ')
                    .addArg(bitRate !== undefined ? `--bit-rate ${bitRate}` : undefined, ' ')
                    .addArg(timeLimit !== undefined ? `--time-limit ${timeLimit}` : undefined, ' ')
    ...
    screenrecordScreenshot(dirName: string, displayId = 0) {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`screenrecord screenshot --display ${displayId} ${dirName}`);
            });
    }
    ...
    screenrecordWebrtcStart(fps = 60) {
        return this.connection()
            .then((conn) => {
                return new ScreenrecordWebrtcStartCommand(conn, this.parser)
                    .execute(fps);
            });
    }
    ...
    screenrecordWebrtcStop() {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`screenrecord webrtc stop`);
            });
    }
    ...
    unfold() {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`screenrecord unfold`);
            });
    }

    multidisplayAdd(id: number, width: number, height: number, dpi: number, flag = 0) {
        return this.connection()
            .then((conn) => {
                return new VoidCommand(conn, this.parser)
                    .execute(`multidisplay add ${id} ${width} ${height} ${dpi} ${flag}`);
            });
    }
